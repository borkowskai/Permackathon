document.addEventListener('DOMContentLoaded', function(){
    ...
    hxr.addEventListener('readystatechange', function(event){
        if (event.currentTarget.readyState == 4 && event.currentTarget.status == 200) {
            ...
            let taskDataBaseArray  = taskDataBaseJSON.results ; //??? -------------------------------------------

            for (let i = 0; i < taskDataBaseArray.length; i++) {

                // récupération des données //

                let maTache                 = taskDataBaseArray[i].tache;
                let monIdTache              = taskDataBaseArray[i].idtache;
                let maDeadline              = taskDataBaseArray[i].deadline;
                let monCreateurTache        = taskDataBaseArray[i].createurtache;
                let monResolverTache        = taskDataBaseArray[i].resolvertache;
                let monIndiceImportance     = taskDataBaseArray[i].indiceimportance;
                let maLocationTache         = taskDataBaseArray[i].locationtache;
                ...
                // chargement des éléments pour le css //
                    ...
                    maDivDropTask.setAttribute('id', "block--" + monIdTache);
